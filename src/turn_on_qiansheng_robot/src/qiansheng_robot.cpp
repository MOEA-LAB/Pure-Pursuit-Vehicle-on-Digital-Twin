...
***************************************/
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::TwistStampedConstPtr &twist_aux)
{
  // ROS_INFO("linear: %lf, angular: %lf", twist_aux->twist.linear.x, twist_aux->twist.angular.z);
  ...
  // 机器人x轴的目标线速度
  transition = 0;
  transition = twist_aux->twist.linear.x * 1000; // 将浮点数放大一千倍，简化传输
...
void turn_on_robot::Publish_ImuSensor()
{
  ...
  // double yaw = Mpu6050_Data.gyros_y_data << 16;
  // yaw += Mpu6050_Data.gyros_x_data & 0xffff;

  int32_t tmp = Mpu6050_Data.gyros_y_data;
  tmp = tmp << 16;
  tmp |= Mpu6050_Data.gyros_x_data;
  // tmp = 3141.5926/4;
  double yaw = (double)tmp / 1000;

  // 只通过yaw, 即绕z的旋转角度计算四元数，用于平面小车。返回四元数
  //  Imu_Data_Pub.orientation = tf::createQuaternionMsgFromYaw(yaw);

  Imu_Data_Pub.orientation_covariance[0] = 1e-6; // Three-axis attitude covariance matrix //三轴姿态协方差矩阵
  Imu_Data_Pub.orientation_covariance[4] = 1e-6;
  ...
  Imu_Data_Pub.angular_velocity_covariance[0] = 1e-6; // Triaxial angular velocity covariance matrix //三轴角速度协方差矩阵
  ...
  // tf::Quaternion quat;
  // tf::quaternionMsgToTF(Imu_Data_Pub.orientation, quat);
...
void turn_on_robot::Publish_Odom()
{
  ...
  odom.pose.pose.position.z = 0;          // Robot_Pos.Z;
...
void turn_on_robot::Control()
{
  ...
  outlog.open("/home/firefly/lqc/myrobot_car/turn_on_robotlog.txt", ios::out | ios::trunc);
  ...
  while (ros::ok())
  {
                                       ...
                                       // 通过串口读取并校验下位机发送过来的数据，然后数据转换为国际单位
    {
      ...
      tf::Quaternion quat;
      tf::quaternionMsgToTF(Mpu6050.orientation, quat);
      double roll, pitch, yaw;                      // 定义存储r\p\y的容器
      tf::Matrix3x3(quat).getRPY(roll, pitch, yaw); // 进行转换

      outlog << _Now << ", " << Robot_Pos.X << ", " << Robot_Pos.Y << ", " << Robot_Pos.Z << ", " << roll << ", " << pitch << ", " << yaw << endl;
...
turn_on_robot::turn_on_robot() : Sampling_Time(0), Power_voltage(0)
{
  ...
  private_nh.param<std::string>("usart_port_name", usart_port_name, "/dev/ttyS4");   // Fixed serial port number //固定串口号
  ...
  // private_nh.param<std::string>("gyro_frame_id",    gyro_frame_id,    "base_footprint"); //IMU topics correspond to TF coordinates //IMU话题对应TF坐标

  odom_frame_id = "odom";
  robot_frame_id = "base_link";
  gyro_frame_id = "base_link";
  ...
  // 速度控制命令订阅回调函数设置
  Cmd_Vel_Sub = n.subscribe("/cmd_vel", 5, &turn_on_robot::Cmd_Vel_Callback, this);
